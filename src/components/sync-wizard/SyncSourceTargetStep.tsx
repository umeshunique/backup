...
interface SyncSourceTargetStepProps {
  state: SyncWizardState;
  onUpdate: (updates: Partial<SyncWizardState>) => void;
}

export function SyncSourceTargetStep({ state, onUpdate }: SyncSourceTargetStepProps) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-muted-foreground">
        Choose source and target server and database. Sync can be bidirectional; both sides will be reconciled
        according to your conflict and rollback settings.
      </p>
          ...
          <CardContent className="p-5 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">First endpoint</p>
              </div>
            </div>
        ...
        <Card className="border-2 border-violet-500/20 bg-violet-500/5">
          <CardContent className="p-5 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-violet-700 dark:text-violet-300">Target</h3>
                <p className="text-xs text-muted-foreground">Second endpoint</p>
              </div>
            </div>
            <div className="space-y-2">
              <Label className="text-sm">Server</Label>
      ...
      {isSameEndpoint && (
        <div className="rounded-lg border border-amber-500/50 bg-amber-500/10 px-4 py-3 text-sm text-amber-800 dark:text-amber-200">
          Source and target are the same. Choose different server or database for sync.
        </div>
      )}
    </div>
  );
