...
import { Sparkles, Database, Server as ServerIcon, Plus } from 'lucide-react';
...
export function DataGeneratorPage() {
  ...
  const canGenerate = selectedServer && selectedDatabase;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Generate test data: random, pattern-based, or from template. Bulk insert with FKs.
            </p>
          </div>
        </div>
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-sm mb-6">
              Add a database server to generate test data. Go to Servers to add your first connection.
            </p>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {canGenerate && selectedServer && selectedDatabase && (
        <DataGenerator
          server={selectedServer}
          database={selectedDatabase}
          onNoServers={() => setActiveTab('servers')}
        />
      )}
      ...
      {selectedServerId && selectedServer && !selectedDatabase && databases.length > 0 && (
        <Card className="border-amber-500/30 bg-amber-500/5">
          <CardContent className="py-6 text-center text-sm text-muted-foreground">
            Select a database above to generate test data.
          </CardContent>
        </Card>
      )}
